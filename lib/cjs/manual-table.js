...
var isEqual = require('./isEqual-6bb5dcd3.js');
...
function baseIsMatch(object, source, matchData, customizer) {
  ...
  while (++index < length) {
    ...
    if (noCustomizer && data[2]) {
      ...
      var stack = new isEqual.Stack;
      if (customizer) {
        var result = customizer(objValue, srcValue, key, object, source, stack);
      }
      if (!(result === undefined
            ? isEqual.baseIsEqual(srcValue, objValue, COMPARE_PARTIAL_FLAG | COMPARE_UNORDERED_FLAG, customizer, stack)
            : result
          )) {
        return false;
      }
...
function isStrictComparable(value) {
  return value === value && !isEqual.isObject(value);
}
...
function getMatchData(object) {
  var result = isEqual.keys(object),
...
function isSymbol(value) {
  return typeof value == 'symbol' ||
    (isEqual.isObjectLike(value) && isEqual.baseGetTag(value) == symbolTag);
}
...
function hasPath(object, path, hasFunc) {
  ...
  return !!length && isEqual.isLength(length) && isEqual.isIndex(key, length) &&
    (isEqual.isArray(object) || isEqual.isArguments(object));
}
...
function baseMatchesProperty(path, srcValue) {
  ...
  return function(object) {
    ...
    return (objValue === undefined && objValue === srcValue)
      ? hasIn_1(object, path)
      : isEqual.baseIsEqual(srcValue, objValue, COMPARE_PARTIAL_FLAG$1 | COMPARE_UNORDERED_FLAG$1);
  };
}
...
function baseForOwn(object, iteratee) {
  return object && _baseFor(object, iteratee, isEqual.keys);
}
...
function createBaseEach(eachFunc, fromRight) {
  return function(collection, iteratee) {
    ...
    if (!isEqual.isArrayLike(collection)) {
...
function baseMap(collection, iteratee) {
  var index = -1,
      result = isEqual.isArrayLike(collection) ? Array(collection.length) : [];
...
function map(collection, iteratee) {
  var func = isEqual.isArray(collection) ? _arrayMap : _baseMap;
  ...
  _rollupPluginBabelHelpers._createClass(ManualTable, [{
    ...
    value: function componentDidUpdate(prevProps, prevState) {
      ...
      if (!isEqual.isEqual(this.props.defaultFilter, prevProps.defaultFilter)) this.setState({
        filter: _rollupPluginBabelHelpers._objectSpread2({}, this.state.filter, {}, this.props.defaultFilter)
      });
      if (!isEqual.isEqual(this.props.defaultSort, prevProps.defaultSort)) this.setState({
        sort: _rollupPluginBabelHelpers._objectSpread2({}, this.state.sort, {}, this.props.defaultSort)
      });
    ...
    value: function getData() {
      ...
      this.props.getData(_rollupPluginBabelHelpers._objectSpread2({}, this.state.filter), _rollupPluginBabelHelpers._objectSpread2({}, this.state.sort), page, this.props.pageSize, function () {
